#Run scripts:
#1. Make script executable: chmod +x create_user_attack.sh
#2. Run script: sudo ./create_user_attack.sh
#3. Check logs: sudo tail -f /var/log/auth.log

#!/bin/bash
CSV_FILE="/home/vagrant/attack.csv"

#************ Column Names *************#
C1="ts"
C2="group_id"
C3="hostname"
C4="mitre_attack_technique"
C5="ip"
C6="start_time_date"
C7="end_time_date"

if ! [ -f $CSV_FILE ]; then
    echo "\"$C1\",\"$C2\",\"$C3\",\"$C4\",\"$C5\",\"$C6\",\"$C7\"" >> $CSV_FILE
fi

TIMESTAMP=$(date +"%Y-%m-%dT%H:%M:%SZ")
START_TIME_DATE=$(date -u +"%Y-%m-%dT%H:%M:%S.%3NZ")
TACTIC_ID="T1562.001"
HOSTNAME=$(hostname)
HOST_IP=$(/sbin/ip -o -4 addr list eth0 | awk '{print $4}' | cut -d/ -f1)
GROUP_NUMBER=$(echo "$HOST_IP" | cut -d. -f3)

RAND_PORT=$(( (RANDOM % 1000) + 40000 ))  # e.g., 40000-40999

EXISTS=$(sudo iptables -L INPUT -n --line-numbers | grep "tcp dpt:$RAND_PORT" || true)

if [ -z "$EXISTS" ]; then
    sudo iptables -A INPUT -p tcp --dport "$RAND_PORT" -j ACCEPT
else
    logger "[ATTACK_LOG][$TIMESTAMP] Skipped firewall rule (already exists): tcp port $RAND_PORT"
fi

sleep 1  # Simulated dwell time

sudo iptables -D INPUT -p tcp --dport "$RAND_PORT" -j ACCEPT

END_TIME_DATE=$(date -u +"%Y-%m-%dT%H:%M:%S.%3NZ")

echo "$TIMESTAMP,$GROUP_NUMBER,$HOSTNAME,$TACTIC_ID,$HOST_IP,$START_TIME_DATE,$END_TIME_DATE" >> "$CSV_FILE"