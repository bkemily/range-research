...
$targetUser = "acctuser_$randId"
$defaultPass = "P@ssw0rd!"

# === Randomize Account Manipulation Action ===
$actions = @("AddToAdministrators", "EnableAccount", "ResetPassword")
$action = $actions | Get-Random
$success = 0

# === Create User First (if not exists) ===
try {
    $exists = (net user $targetUser 2>$null) -ne $null
    if (-not $exists) {
        cmd /c "net user $targetUser $defaultPass /add /active:no"
    }

    switch ($action) {
        "AddToAdministrators" {
            cmd /c "net localgroup Administrators $targetUser /add"
            $success = 1
        }
        "EnableAccount" {
            cmd /c "net user $targetUser /active:yes"
            $success = 1
        }
        "ResetPassword" {
            $newPass = "NewPass$((Get-Random -Minimum 1000 -Maximum 9999))!"
            cmd /c "net user $targetUser $newPass"
            $success = 1
        }
    }
} catch {
...
# === Cleanup: Remove user to avoid clutter ===
try {
    cmd /c "net user $targetUser /delete"
} catch {
    # silently fail if user wasn't created or already removed
}